...
class Filter {
    ...
    checkExcludeTag(tags, id) {
        ...
        for (const tag of tags) {
            for (const str of _setting_Settings__WEBPACK_IMPORTED_MODULE_4__["settings"].notNeedTag) {
                    ...
                    if (tag.toLowerCase().includes(notNeed)) {
                        ...
                        if (words.length > 1) {
                            ...
                            if (words.some((word) => word.toLowerCase() === notNeed)) {
                                // 定制：显示被排除的原因
                                _Log__WEBPACK_IMPORTED_MODULE_1__["log"].warning(`id ${id || '未知'} 被排除，因为排除了 tag：${tag}`);
                                return false;
                            }
                        }
                        else {
    ...
    checkBlockList(userId, id) {
        ...
        const result = _setting_Settings__WEBPACK_IMPORTED_MODULE_4__["settings"].blockList.includes(userId);
        // 定制：显示被排除的原因
        if (result) {
            _Log__WEBPACK_IMPORTED_MODULE_1__["log"].warning(`id ${id || '未知'} 被排除，因为用户阻止名单中排除了该用户：${userId}`);
        }
        return !result;
    }
    checkSize(size) {
    ...
    checkBlockTagsForSpecificUser(userId, tags, id) {
        ...
        // 定制：显示被排除的原因
        const checkResult = _BlockTagsForSpecificUser__WEBPACK_IMPORTED_MODULE_7__["blockTagsForSpecificUser"].check(userId, tags);
        if (checkResult.result) {
            _Log__WEBPACK_IMPORTED_MODULE_1__["log"].warning(`id ${id || '未知'} 被排除，因为屏蔽了用户 ${userId} 的 tag：${checkResult.tag}`);
        }
        // 对结果取反
