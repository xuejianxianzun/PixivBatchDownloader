...
有些用户想修改 `{date}` 命名标记的结果，比如 `yyyy-mm-dd` 改成 `yymmdd`。

众口难调啊，完美的解决办法大概需要新增个输入框，让用户自己输入想要的日期格式。但是这样也挺麻烦的，我不想做。干脆让有需要的用户自己去源码里修改日期格式吧！不过之前并没有设置日期格式的地方，所以我现在新增了解析日期格式的类 `DateFormat.ts`。用户修改日期格式就可以得到想要的结果。
