...
### 🔧预览搜索页面的抓取结果时，作品会分页显示，以降低性能压力

之前所以作品都是直接显示在页面上的，当数量很多时，页面会非常卡顿。现在改为分页显示。

### 🔥移除了“预览搜索页面的抓取结果”的“上限”设置

因为“预览搜索页面的抓取结果”已经改为了分页显示所以没有必要设置上限了。

### 🐞修复问题：特定情况下，保存汇总的简介文件之后，会导致图片下载失败

触发这个 bug 的条件如下：
- 生成文件名时，只有一层文件夹，例如 `{user_id}_{user}/{id}_{title}`。默认的命名规则有两层文件夹，所以不会触发这个 bug。
- 启用“保存作品简介”并选择“汇总到一个文件”。

表现：
- 下载器错误的用整个文件名作为简介文件的目录，也就是 `文件夹/文件名（含扩展名）/简介.txt`。注意：图片的文件名如 `149205565_p0_大道寺知世.jpg` 会被创建为文件夹
- 之后保存图片时，图片名字和文件夹的名字是同名的，并且无法覆盖文件夹失败，导致浏览器一直无法成功保存文件，文件无法完成下载。
...
如果用户的操作是：
1. 鼠标进入作品缩略图元素。此时下载器会在缩略图上显示一些按钮
2. 鼠标短暂经过按钮
3. 鼠标从按钮上移出并落入缩略图元素。此时不会触发预览作品的详细信息功能，也就是不会显示详情面板。

PS：在这个情况里，预览作品（的图片）的功能是正常触发的。

根因是 `PreviewWorkDetailInfo` 在再次进入同一作品缩略图时，会因 `id === this.workId` 直接返回。在上面的操作里：

1. 鼠标进入缩略图是，会保存此时的 workId
2. 鼠标经过按钮时，会触发缩略图的离开事件，进而取消详情面板的延迟显示；
...
另外，我更新了在移动端页面里查找作品缩略图的代码，查找更准确、也覆盖到了很多之前没有覆盖到的地方，这样才能让全选功能发挥作用。

### ✨新增功能：手动排除作品

![](notes/images/20260904_023117.png)
...
![](notes/images/20260828_182557.png)

在多图作品页面里，下载器会在大图下方添加缩略图列表。

有时用户可能想选择其中的一部分图片然后下载它们，所以我添加了一些按钮来实现这个功能。

### ⚙️优化了“自动开始下载”设置

之前即使用户关闭了“自动开始下载”，快速下载任务也总是会自动开始下载。

现在我区分了“普通下载任务”和“快速下载任务”，你可以分别设置它们是否自动开始下载。

### ✨在预览作品里新增子选项：允许预览图遮挡缩略图

该选项默认未启用。现在预览作品区域默认不会遮挡缩略图，除非你启用这个选项。

之前当你预览横图时，预览图经常会遮挡住缩略图，有时这会影响用户体验。现在预览图默认不会遮挡住缩略图了，体验更好，但有时这会导致预览图的尺寸比原来更小。

如果你想让预览图和以前一样：尽量不缩小，且允许它遮挡住缩略图，可以启用此开关。

### ✨在预览作品里添加了更小的图片尺寸：小图(540px)

之前下载器在预览作品时默认使用普通尺寸（1200px），有很多图片的体积达到了 1.5 MB，在弱网时加载比较慢，所以我添加了更小的尺寸：小图(540px)。大部分小图的体积只有 50 KB 左右，你可以根据需要选择这个尺寸。

### ✨在预览作品时，按 V 可以查看它的原始比例图片

预览作品时显示的图片是缩略图，如果你想以 1:1 的比例查看原图，可以按 `V` 查看原图，再次按 `V` 可以关闭原图。

*其实这个查看原图的功能就是“在缩略图上长按鼠标右键时显示大图”，只不过添加了一个新的调用方式。*

另外，这次我为查看原图功能添加了两个新的快捷键：
- 按 `B` 收藏这个作品；
- 按 `V` 关闭原图。

### ✨在搜索用户的页面里，适配了“显示更大的缩略图”的样式
...
PS：在下载器不支持的页面里没有这个按钮，此时快捷键没有实际效果。
...
[](notes/images/20260826_125935.png)

*其实我先用了在浏览器里注册扩展快捷键的方法来做这个功能，但是这个方法有很多不便，后来返工改成了由下载器自己管理。*

文档：notes/添加自定义快捷键功能.md
...
- “导出日志”改为“自动导出日志”。这是为了与新增的“手动导出日志”按钮区分。
...
之前在移动端页面里，下载器顶部的标题栏、搜索栏和导航栏占据了太多高度，导致下方内容区域的高度很小，使用起来很不方便。
...
之前当用户从普通分类里进入 R-18 分类时，如果设置的值是 500，会自动变成 100。之后即使返回普通分类，也还是 100。这会让用户困惑，因为数字自动变小了。现在这个值不会变化了，所以用户可以使用大于最大值的数字。

### 😊优化体验：文件名超长时，之前可能会只保留 id，现在可以保留更多文字了

当需要截断文件名时，在截断后的内容里，文件名里可能没有包含 `{id}` 的内容。这通常是因为用户在命名规则里把 `{id}` 放到了末尾。为了保证文件名不重复，之前下载器会强制把文件名设置为 `{id}`，但这丢失了其他信息。现在改为只把末尾的一些文字替换为 `{id}`，保留了更多信息。

### 😊优化了一些帮助内容

### 🌍优化了 i18n 文本的翻译

`src/ts/langText.ts` 里保存着下载器的 i18n 文本，以前有些翻译不准确。主要的原因是：以前 AI 没有流行的时候，我是用谷歌翻译来把简体中文文本翻译成其他语言的，但是翻译质量并不好。现在我让 AI 重新进行了翻译，修正和优化了一些翻译结果。
...
![](./notes/images/20260722_193047.webp)
...
一些系列小说允许用户置换单词。这是由小说作者在系列资料里设置的，允许用户把特定单词替换成自定义文字。常见于角色名，例如用 `△`、`〇`代表两个角色的名字，这样用户可以把它们替换成自己想要的名字。
...
```
⚠️作品 ID: 99381250_p1, 异常文件名: D:\download\551dd332-73c9-422d-b2a9-7664b55c1427.png
```
...
```
你还需要在“文件夹和文件的名字”-“图像作品的命名规则”里添加 /{multi_image_folder}/ 才能实现这个效果。你可以点击“帮助”按钮查看详细说明。
```
...
![PixivBatchDownloader screenshot](./notes/images/20260530_040343.png)
