...
-  23316 条小说结果（中文），文件体积为 373 MB (391,208,847 字节)，平均每条结果的体积约为 16 KB。
-  4512731 条下载记录，文件体积为 173 MB (181,929,686 字节)，平均每条结果的体积约为 0.04 KB。
...
插画和下载记录的体积比较平均，可预测。但是小说的体积比较悬殊，有的只有几 KB，但也有很多有 100 KB 左右。

导入的时候，体积超过 512 MB 导致导入失败了，我不得不写了一份代码用来把这个大文件分割成多个小文件。但是用户不会这个方法，所以我需要限制导出的抓取结果的体积。
...
2. 在文件生成后检查 Blob 对象的 byteLength，如果超大就设置分割标记，重新生成。但是这样不太优雅。
...
这个字符串的 length 是 15，但是字节数是 23。可以用 `textEncode.encode(string).length` 获取 byteLength，这不是技术限制，但是我担心频繁的 encode 会对性能造成影响。尚不清楚会增加多少导出时间。

### 获取屏蔽设置（Mute）失败时不会中止抓取

之前如果获取屏蔽设置（Mute）失败（最常见的原因是用户没有登录账号，就会出现 401 错误），Mute 模块会抛出错误导致抓取中止。现在解除这个限制，获取失败也可以继续抓取。
