...
修复了关注的新作品页面最多只会抓取 100 页的问题。最大页数上限改为 250。
...
打开下载面板，可以看到“手动选择作品“按钮，点击它就可以开始选择作品了。它还可以暂停选择、继续选择。（你也可以使用对应的快捷键 `Alt + S`，效果和你点击这个按钮是一样的）。

当你选择了作品之后，作品上会添加一个“😊”标记，并且下载面板里会显示“抓取选择的作品”按钮。点击这个按钮可以抓取你选择的作品。

当抓取完成之后，选择的作品会被清空，以便你可以开始新的选择。

**提示：**

- 你可以通过“暂停选择”、“继续选择”来进行多次选择，最后统一抓取。
- “抓取选择的作品”按钮会显示选择的作品数量。
- 下载器的过滤条件对手动选择的作品也会生效。所以你选择的某些作品可能不会被下载。
- 为防止误操作（因为点击链接会跳转到其他页面），所以进入选择模式之后，页面上的链接都是无法打开的。你可以点击“暂停选择”按钮（或按 `Esc` 键），使链接可以打开。
- 当你进入其他页面时，**可能**会丢失选择的作品列表。所以在你进入其他页面之前，最好先抓取选择的作品。原因：有时候进入某些页面是不需要重新加载网页的（浏览器的刷新按钮不会变化），这种情况下不会丢失已选择的作品。如果重新加载了网页（浏览器的刷新按钮发生变化）则会丢失已选择的作品。
