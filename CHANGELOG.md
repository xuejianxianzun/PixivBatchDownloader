...
之前，下载失败的文件会生成一个 txt 文件下载，并把它标记为已下载。

但是现在有了断点续传，在所有文件下载完成后，断点续传会清空这个任务的数据，这时自然也清空了下载出错的文件的数据。这样就导致断点续传无法恢复这些出错的任务。

现在进行了修改，下载失败的文件不会被标记为已下载。所以断点续传会保留任务的数据，可以再次尝试下载这些失败的文件。
