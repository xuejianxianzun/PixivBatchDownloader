...
一般不需要开启，因为文件名太长的话，Chrome 可以自动截断，不会导致保存失败。

特殊情况下，比如你把下载位置设置到 NAS 或云端硬盘，Chrome 可能不会自动截断文件名，导致保存失败。

所以当你确定问题是文件名太长导致的，才需要启用这个设置。
...
最近 漫画系列页面 改版了，但是只有一部分人是新版，还有一部分人是旧版，所以需要兼顾这两种情况。
