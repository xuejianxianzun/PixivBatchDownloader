...
搜索页面在获取了列表页之后，会开始获取作品数据。

作品数据是会应用过滤条件的，例如 1000 个作品可能只有 50 个符合要求。有些用户看到符合要求的数量很久才变化一下，以为抓取卡住了。

所以现在增加了“还剩多少作品没有抓取”的显示，避免用户产生疑惑和焦虑。
