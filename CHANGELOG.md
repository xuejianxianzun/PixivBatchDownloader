...
此版本暂时封存，不会发布。后续版本仍然以 Manifest V2 为基础，等到时机成熟了再合并这个版本的内容（V3 的内容）。

这是因为 V3 需要 Chrome 88 版本才能支持，很多国内套壳浏览器，以及手机上的 yandex 浏览器尚无法支持。

手机上的 yandex 浏览器目前内核版本已经是 88，但是却提示不支持 V3 的扩展，无法使用。

### 扩展迁移到 Manifest V3

这次升级的主要原因是 Chrome webstore 的扩展管理页提示：使用 Manifest V3 可以加快审核速度。

此外，使用 Manifest V3 还有助于保护隐私，提高性能。而且在未来 V3 会逐渐取代 V2（不过我估计需要很长时间，因为 V2 的扩展程序的数量实在是太多了）。
