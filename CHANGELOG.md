...
之前的代码是面向过程编程，按照传统流程编写一个个的函数来解决需求。

面向对象编程就是将需求抽象成一个类，然后针对这个对象分析其属性与方法。
...
如果 cfg 里的一些属性只在特定类里使用，就把它挪到特定类里。
...
2. 拆分成小的类
3. 函数做了多件事的，拆分
4. 使用异常替代返回错误码，将错误处理代码从主路径中分离。如果你打算在方法中返回 null 值，不如抛出异常，或是返回空对象或特例对象。
