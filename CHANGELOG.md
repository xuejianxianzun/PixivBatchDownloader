...
之前生成文件名的时候，每次生成都会对命名规则进行非法字符的处理，这造成了效率上的浪费。

现在这个处理被放到了 Settings 类里，只在修改命名规则时执行一次。生成文件名的时候不必执行这个处理，提高了效率。

现在在本机上进行的效率测试：

使用命名规则 `{p_title}/{id}-{user}-{user_id}-{tags_transl_only}` 对抓取的 11377 个文件预览文件名，花费的时间约为 `368 ms`。

还有优化空间，但是因为已经够快了，所以没再继续优化。继续优化的话某些部分的代码会变得复杂，所以显得不是很有必要。
