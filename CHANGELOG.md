...
在大多数页面里都添加了这个功能。现在你可以手动选择页面上的任意作品进行下载！

快来试试吧~

**提示：**

- 为防止误操作（因为点击作品时默认会跳转到作品页面），所以进入选择模式之后，页面上的链接都是无法打开的。此时你不能点击链接跳转到其他页面，也不能点击“加载更多”按钮，不能翻页。
...
1. 打开下载面板，点击“抓取选择的作品”。这会退出选择模式，并且开始抓取作品。
2. 按键盘的 `Esc` 键。这只会退出选择模式，但不会抓取作品。
