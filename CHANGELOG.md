...
初始化时，settingChange 触发次数过多，需要优化

右键预览大图存在问题：有时候右键直接出现大图了，等待时间似乎失效了
...
现在 `Lang` 模块默认使用网页上的语言类型，并且不再阻塞其他模块的执行。等设置读取完毕之后，`Lang` 模块获取用户设置的语言，再去更新一次界面文本（如果语言类型不同的话）。

新的逻辑需要界面文本能动态修改，所以做了这个优化。
