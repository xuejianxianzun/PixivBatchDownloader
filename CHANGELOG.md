...
其实就是用户黑名单。开启这个选项，然后输入要屏蔽的用户的 id，这样就不会下载这些用户的作品。
...
### 新增设置项：可以设置下载器收藏作品时的行为

新增了一个设置。当下载器把作品添加到收藏时，可以设置是否附带 tag，是否为公开收藏。

这会影响很多通过下载器添加收藏的操作（区别于点击 pixiv 原本的收藏按钮）。
