...
发生变化的地方是插画、漫画通用数据（`src\ts\modules\CrawlResult.d.ts` 里的 `ArtworkCommonData`），里面现在没有 `illustId` 和 `illustTitle` 属性了。

### 新版收藏页面里去掉了一键添加 tag 的按钮

因为“一键添加 tag”这个功能需要获取非公开收藏的作品，但现在新版收藏页面里只会显示公开收藏的，不允许获取非公开收藏的，获取或出错，导致无法添加。所以就去掉了这个按钮。
