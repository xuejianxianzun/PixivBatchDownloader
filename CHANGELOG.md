...
### ⚙️行为变化：当你修改了下载器的设置之后，所有 Pixiv 标签页里的设置都会同步变化

用户经常会打开多个 Pixiv.net 标签页。之前当你在一个标签页里修改了设置之后，其他标签页里的设置不会变化。现在，其他标签页默认会同步变化，所以所有标签页都会使用相同的设置。

⚠️注意：这个行为与之前不同。如果你想保持以前的行为，可以在下面的设置里选择旧版行为。

### ✨新增设置：当你修改设置时，其他标签页是否同步变化

这个设置位于“通用”-“管理设置”分类里，用来决定每个标签页里使用相同的设置，还是保持各自的独立设置。

你可以选择“同步变化”（默认值）或者“保持不变”（旧版行为）。
...
“快捷屏蔽用户”功能：
当你启用了“用户屏蔽名单”功能之后，把鼠标指针停留在任意用户的名字上，下载器就会显示屏蔽他的按钮，点击即可添加屏蔽。

以前屏蔽用户必须手动复制他的 ID 并添加到输入框里，现在方便多了。
...
### 📝下载器导出的一些文件的名字里可能含有时间戳，我修改了时间字符串的格式
