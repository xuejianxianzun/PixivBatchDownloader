...
之前当用户在图片查看器里滚动鼠标滚轮时，只能缩放图片，现在可以切换图片了。具体说明：
- 如果鼠标指针在图片区域之外滚动，就会切换图片
- 如果鼠标指针在图片上滚动，则依然是缩放图片
...
之前 IDData 里只能保存单个作品的 id，所以抓取流程里也只能处理单个作品。如果用户通过手动选择功能或者快速下载按钮来直接下载系列小说，之前只能在抓取流程之外单独处理，用户体验不好。
...
之前需要我在  WorkPublishTime.ts 里手动设置开始 id、结束 id，繁琐而且容易出错，现在改为自动获取，不需要手动配置了。
