...
**测试 4：**（该功能已经基本完成，并略微加大了下载文件时的间隔时间）
...
**测试 1：** 较早期，很多请求之间没有添加间隔时间，导致被警告了。
...
从上面几次测试里得出的粗略结论：
- 平均每分钟 30 - 40 个请求不会被警告。这里说的请求包括 API 和文件。
- 搜索页面的 900 个小说里有 150 个左右系列，每个系列平均有 10 个或更多小说（多的时候可能有 30 个左右）。
- 在大量抓取时，请求数量 / 小说数量（包含列表页里的和系列里的所有小说），大约每个小说会导致发出 2.2 个请求。小说本身的数据是一次，封面图片是一次，这样就占用了 2 次。剩下的额外请求包括：获取系列相关数据的 API、系列封面图、小说里内嵌的图片。看来这些额外的请求相比于小说的数量是很少数（只有 0.2 左右，约为小说数量的十分之一）。
- 如果只从列表页里初始的小说数量（900 个）来算的话，总的请求数量接近初始数量（900）的 5 倍或更多（多的时候可能达到 15 倍）。
