...
其中有个很大的变化是页面里没有 `#root` 元素了，因此下载器的很多功能都受到了影响。
...
我对 `src\style\showLargerThumbnails.less` 的内容也进行了优化。以前所有页面类型里的规则都在同一层级，当需要修改某个页面里的规则时根本找不到对应的规则。而且修改某些规则可能会影响多个页面里的表现。

现在我把每个页面类型里的规则分别保存，不再像以前一样是大杂烩了。
