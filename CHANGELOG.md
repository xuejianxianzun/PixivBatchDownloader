...
### EVT.fire 取消使用 setTimeout 0

之前的版本把触发事件做了 setTimeout 0 处理，解决了某些问题。但是这样修改之后导致一些之前正常的功能出现了异常。

现在想想，本来就不该给触发事件的代码加 setTimeout 的，所以又取消掉了，不知道这会不会引起新的 bug，我尽量测试一下。
