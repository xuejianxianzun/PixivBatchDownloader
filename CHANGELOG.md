...
出现这个提示时，之后应该重试下载此文件，但是今天看到两次这样的提示，原因是 `FILE_FAILED`，实际上最后也没下载到。

## 5.0.0 2020-03-31

### 可以过滤彩色或黑白图片

下载器在获取作品的详细信息时，会向过滤器发送作品缩略图的 url。注意缩略图只是每个作品的第一张图，后面的图片一般没有缩略图。如果启用了“过滤黑白图像”的选项，那么过滤器将会加载缩略图，判断它是不是黑白图片。如果是的话就过滤掉掉这个作品。

**注意：**这个方法并不算完美。虽然程序判断颜色是没错的，但是实际使用中，掺杂了其他因素，就显得不完美了。

1. 可能会误伤（但关系不大）。灰色图片的 R G B 三色应该是相同的颜色，例如 `200 200 200`。但是我加了一个数值为 1 的宽容度，这样类似于 `200 200 201` 的图片也会被认为是黑白图片。所以偶尔会误伤，例如 `https://www.pixiv.net/artworks/80468838` 图中有涂抹一些色块，R G B 值为 `225 226 226`，加了宽容度之后，它会被当作黑白图。之所以说问题不大，是因为这样的图大部分还是黑白的，过滤掉也说得过去。
2. 可能会遗漏（从人的认知出发）。例如对于 `https://www.pixiv.net/artworks/80469066`，如果一个用户不想要这张图，因为这图**看上去**是黑白的。但是这图的背景色有点偏紫色，所以**事实上**这不是黑白图。程序会把它判断为彩色图片。虽然和人的期望不一样，但是也没什么好的办法。
3. 还有一种遗漏的情况：作品的第一张图是彩色，后面的图是黑白。但是因为判断缩略图时用的是第一张图的，所以这个作品不会被认为是黑白图。例如 `https://www.pixiv.net/artworks/80417958`。

在程序的处理上，要注意：

1. 不要再加大宽容度，因为宽容度哪怕再增加 1，引起的误伤都是很可怕的。
2. 抓取时一般有两个阶段要使用过滤器，1 是在获取作品的 id 列表时，先用过滤器过滤一次。2 是在获取每个作品的详细信息时，过滤第二次。过滤黑白图像应该在第二次进行，因为第一次可能获取不到缩略图 url，而且一个作品没有必要检查两次缩略图，只在最后一关检查就行了，否则对效率的影响可能比较大。
