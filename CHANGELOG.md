...
- `{page_id}`：开始抓取时的页面 ID。它的设计目的是用页面 ID 来归纳该页面里的多个作品。例如：在作品页面里抓取相关作品时，这个标记会输出该页面的作品 ID（它只有一个），而非每个作品自己的 ID（多个）。只在以下情况有输出：1. 在作品页面里抓取时，输出该页面的作品 ID。2. 在用户主页和收藏页面里抓取时，输出该页面的用户 ID。3. 在系列页面里抓取时，输出系列 ID。
