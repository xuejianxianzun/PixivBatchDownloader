...
这是为了解决 429 问题的尝试，可以避免 429 问题进行高速抓取。但是它也不是完美的。
...
注意：
1. 因为未登录，无法获取到 R18(G) 作品的数据，只能获取到普通作品的数据。这是这个方法最大的局限性。
2. 因为未登录，所以无法检查作品的收藏状态，不能使用“收藏状态”过滤选项。
3. 因为未登录，你在 Pixiv 账户设置里的屏蔽（Mute）用户或标签的功能也不会生效。

使用场景：
- 适合要抓取的作品全部是普通（全年龄）作品时使用。
