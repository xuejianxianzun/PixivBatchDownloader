...
- `{page_id}`：开始抓取时的页面 ID。目前有两种输出：在用户主页和收藏页面里抓取时，输出该页面的用户 ID；在作品页面里抓取时，输出该页面的作品 ID。它的设计目的是用一个 ID 来归纳多个作品。例如：在作品页面里抓取相关作品时，这个标记会输出该页面的作品 ID（唯一），而非每个作品自己的 ID。


PS：在系列页面里，`{page_id}` 是无效的。虽然理论上可以让它输出系列 ID，但我故意没有这么做，因为：
1. 此时可以使用 `{series_id}` 来输出系列 ID。
2. 由于系列页面里的所有作品都属于同一个系列，所以`{series_id}` 已经可以归纳该页面里的作品，不需要 `{page_id}` 来重复做这件事情。
3. 在系列页面里，用户很可能会使用 `{series_id}`。如果用户同时使用了 `{page_id}`，可能会重复输出系列 ID，所以我故意不让 `{page_id}` 输出系列 ID。
