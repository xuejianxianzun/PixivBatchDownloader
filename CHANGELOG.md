...
它替代了旧的设置：“为每个作品建立单独的文件夹”，并且使用方法也有所不同：在命名规则里使用 `{multi_image_folder}` 代表该设置的效果。
...
因此移除了一个旧的设置：`createFolderBySl`（使用 sl 数据创建一侧文件夹）。
