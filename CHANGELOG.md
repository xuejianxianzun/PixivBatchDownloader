...
下载器会在作品缩略图上和作品页面内显示一个复制按钮，点击它就可以复制作品的一些数据。你可以自定义要复制的数据和格式。

相关设置：“更多”-“增强”分类里的“显示复制按钮”。

在作品页面内，你可以使用快捷键 `Alt` + `C` 来复制当前作品的数据。

--------------

这个功能在移动端（Android 系统的浏览器里）存在兼容性问题：虽然可以复制图片 + 文字的混合内容，但是图片粘贴到通讯软件里时，无法显示图片内容，只能显示为一个带 OBJ 字符的方块。所以我改成了在移动端只会复制纯文本内容，不会复制缩略图。
