...
offline 分支是打包用于发布到 github 上的，用于离线安装的。

offline 分支会检查 github 上是否发布了新版本，但是 master 分支上不会检查新版本。
...
当鼠标停留在作品的缩略图上时，下载器会显示更大尺寸的缩略图。

- 你可以设置大缩略图的尺寸。
- 大缩略图可能会遮挡住作品原本的缩略图。在大缩略图上点击鼠标左键可以关闭它。
...
这个版本里，区分了提交到商店的在线安装包（不检查 github 上的更新），和发布在 github 上的离线安装包（会检查 github 上的更新）
