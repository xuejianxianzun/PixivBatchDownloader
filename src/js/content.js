...
async function addTag(index, addList, tt, addTagBtn) {
    await addBookmark(addList[index].id, addList[index].tags, tt, addList[index].restrict);
...
function getRatioSetting() {
    ...
    if (ratioType === '1') {
        ...
        addOutputInfo('<br>' + xzlt('_设置了宽高比之后的提示', xzlt('_输入宽高比') + ratioType));
    }
    return true;
}
// 检查作品是否符合宽高比条件
...
function startGet() {
    ...
    if (pageType === 1) {
        ...
        addOutputInfo(xzlt('_tag搜索任务开始', parseInt(xzForm.setFavNum.value), wantPage));
    ...
    fetch(url)
        ...
        if (downRelated) {
            ...
            outputArea.innerHTML =
                nowTips +
                    '<br>' +
                    xzlt('_tag搜索页已抓取多少页', tagPageFinished, wantPage, startpageNo + listPageFinished - 1);
            // 每抓取完一页，判断任务状态
            if (tagPageFinished === wantPage) {
                // 抓取完了指定的页数
                allowWork = true;
                addOutputInfo('<br>' +
                    xzlt('_tag搜索页任务完成1', document.querySelectorAll(tagSearchListSelector).length) +
                    '<br><br>');
                ...
                allowWork = true;
                addOutputInfo('<br>' +
                    xzlt('_tag搜索页任务完成2', document.querySelectorAll(tagSearchListSelector).length) +
                    '<br><br>');
                tagSearchPageFinished();
                return false;
            }
                ...
                // 任务被用户中断
                addOutputInfo('<br>' +
                    xzlt('_tag搜索页中断', document.querySelectorAll(tagSearchListSelector).length) +
                    '<br><br>');
                interrupt = false;
                tagSearchPageFinished();
                return false;
            ...
            if (pageType === 10 && listIsNewMode === true) {
                ...
                let thisOneInfo = listPageDocument.querySelector(tagSearchDataSelector).dataset.items; // 保存本页的作品信息
                thisOneInfo = JSON.parse(thisOneInfo);
                for (const data of thisOneInfo) {
                    // 检查收藏设置
...
function getListPage2() {
    ...
    allowWork = false;
    addOutputInfo('<br>' + xzlt('_列表抓取完成开始获取作品页', illustUrlList.length));
    if (illustUrlList.length <= 0) {
        addOutputInfo('<br>' + xzlt('_参数不合法1'));
    }
    getListUrlFinished();
    ...
    fetch(url, {
        ...
        if (type2ListType !== 3) {
            if (!hasTag) {
                ...
                else if (type2ListType === 1 || type2ListType === 5) {
                    // 插画 或 动图
    ...
    if (response.ok) {
        ...
        if (pageType === 1 && !downRelated) {
            ...
            if (wantPage === -1 || wantPage > 0) {
                ...
                if (nextId) {
                    getIllustPage('https://www.pixiv.net/member_illust.php?mode=medium&illust_id=' +
                        nextId);
                }
                else {
                    // 没有剩余作品
...
function testExtName(url, length, imgInfoData) {
    ...
    function nextStep(bool) {
        ...
        addImgInfo(imgInfoData.id, url, imgInfoData.title, imgInfoData.tags, [], imgInfoData.user, imgInfoData.userid, imgInfoData.fullWidth, imgInfoData.fullHeight, ext, '', '', {});
        outputImgNum();
        if (length !== undefined) {
