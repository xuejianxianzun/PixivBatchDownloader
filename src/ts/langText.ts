...
- Изображение APNG, ZIP-файл и файл Ugoira: добавьте расширения этих форматов в Icaros, и их миниатюры будут отображаться. Если у вас установлен K-Lite Codec Pack, то Icaros, скорее всего, уже входит в комплект, и его можно найти через поиск в меню "Пуск". Если вы не можете его найти, можно отдельно установить <a href="https://github.com/Xanashi/Icaros/releases" target="_blank">Icaros</a>. Запустите Icaros, включите функцию миниатюр, добавьте <span class="blue">;apng;zip;ugoira</span> в конец списка типов файлов или перетащите файлы этих типов в окно Icaros, чтобы добавить их.<br>`,
  ...
  _命名标记like: [
    ...
    `Like count.`,
    `Like count，作品のいいね数。`,
    `Like count, 작품의 좋아요 수.`,
    `Like count, количество лайков работы.`,
  ],
  _命名标记view: [
    `View count，作品的浏览量。`,
    `View count，作品的瀏覽量。`,
    ...
    `View count，作品の閲覧数。`,
    `View count, 작품의 조회수.`,
    `View count, количество просмотров работы.`,
  ],
  _命名标记pid: [
  ...
  _同时下载多少个文件: [
    ...
    `Скачивать <span class="key">сколько</span> файлов одновременно`,
  ],
  _下载线程数量: [
    `下载线程数量`,
    `下載執行緒數量`,
...
Поэтому имя файла, заданное загрузчиком, теряется, и имя файла становится последним путем в URL. <br>`,
  ...
  _抓取完成后自动开始下载: [
    ...
    `Начать загрузку <span class="key">автоматически</span> после завершения сканирования`,
  ],
  _应用到: [
    `应用到：`,
    `應用到：`,
...
<br>
下载任务分为两种：<br>
- 普通下载任务：下载器的“开始抓取”区域里有多个抓取按钮，点击第一排的抓取按钮通常会建立普通下载任务，只有两个按钮例外：“定时抓取”和“抓取标签列表”。<br>
- 快速下载任务：直接点击下载按钮，或者抓取手动选择的作品。这些行为有更强烈的“立即下载”或“自动下载”的需求，所以以前下载器总是会自动下载它们。现在你也可以把它们设置为不自动开始下载。<br>
<br>
快速下载任务会由这些行为触发：<br>
- 点击作品缩略图上的下载按钮<br>
- 在预览作品、使用图片查看器、查看原图时，点击下载按钮，或者使用快捷键建立下载任务<br>
- 抓取手动选择的作品或图片<br>
- 在作品页面里，点击页面右侧的快速下载按钮<br>
- 在作品页面里，抓取推荐作品<br>
...
- Normal download task: there are multiple crawl buttons in the "Start crawling" area of the downloader. Clicking the crawl buttons in the first row usually creates a normal download task, with only two exceptions: "Timed crawl" and "Crawl tag list".<br>
- Quick download task: directly clicking the download button, or crawling manually selected works. These actions have a stronger need for "download immediately" or "auto-download", so the downloader always used to download them automatically. Now you can also set them not to start downloading automatically.<br>
<br>
Quick download tasks are triggered by these actions:<br>
- Clicking the download button on a work thumbnail<br>
- Clicking the download button or using a hotkey to create a download task while previewing a work, using the image viewer, or viewing the original image<br>
- Crawling manually selected works or images<br>
- Clicking the quick download button on the right side of a work page<br>
...
- 通常ダウンロードタスク：ダウンローダーの「クロールを開始する」エリアには複数のクロールボタンがあります。1列目のクロールボタンをクリックすると、通常は通常ダウンロードタスクが作成されます。例外は「時限クロール」と「タグのリストをクロール」の2つのボタンだけです。<br>
- クイックダウンロードタスク：ダウンロードボタンを直接クリックするか、手動で選択した作品をクロールします。これらの操作には「すぐにダウンロード」または「自動ダウンロード」という強いニーズがあるため、以前はダウンローダーは常に自動的にダウンロードしていました。現在は、自動的にダウンロードを開始しないように設定することもできます。<br>
<br>
クイックダウンロードタスクは次の操作でトリガーされます：<br>
- 作品のサムネイル上のダウンロードボタンをクリックする<br>
- 作品をプレビューするとき、画像ビューアーを使用するとき、または元画像を表示するときに、ダウンロードボタンをクリックするか、ショートカットキーを使用してダウンロードタスクを作成する<br>
- 手動で選択した作品または画像をクロールする<br>
- 作品ページの右側にあるクイックダウンロードボタンをクリックする<br>
...
- 일반 다운로드 작업: 다운로더의 "긁어오기 시작" 영역에는 여러 크롤링 버튼이 있습니다. 첫 번째 줄의 크롤링 버튼을 클릭하면 일반적으로 일반 다운로드 작업이 생성됩니다. 예외는 "시간 제한 크롤링"과 "태그 긁어오기" 두 버튼뿐입니다.<br>
- 빠른 다운로드 작업: 다운로드 버튼을 직접 클릭하거나 수동으로 선택한 작품을 크롤링합니다. 이러한 동작은 "즉시 다운로드" 또는 "자동 다운로드"에 대한 요구가 더 강하므로 이전에는 다운로더가 항상 자동으로 다운로드했습니다. 이제 자동으로 다운로드를 시작하지 않도록 설정할 수도 있습니다.<br>
<br>
빠른 다운로드 작업은 다음 동작으로 트리거됩니다:<br>
- 작품 썸네일의 다운로드 버튼 클릭<br>
- 작품 미리 보기, 이미지 뷰어 사용, 원본 이미지 보기 시 다운로드 버튼을 클릭하거나 단축키를 사용하여 다운로드 작업 생성<br>
- 수동으로 선택한 작품 또는 이미지 크롤링<br>
- 작품 페이지 오른쪽의 빠른 다운로드 버튼 클릭<br>
...
- Обычная загрузка: в области «Начать сканирование» загрузчика есть несколько кнопок сканирования. Нажатие кнопок сканирования в первом ряду обычно создает обычную задачу загрузки, за исключением двух кнопок: «Сканирование по таймеру» и «Сканировать список тегов».<br>
- Быстрая загрузка: нажатие кнопки загрузки напрямую или сканирование вручную выбранных работ. Эти действия имеют более сильную потребность в «немедленной загрузке» или «автоматической загрузке», поэтому раньше загрузчик всегда загружал их автоматически. Теперь вы также можете настроить их так, чтобы они не начинали загружаться автоматически.<br>
<br>
Быстрая загрузка запускается следующими действиями:<br>
- Нажатие кнопки загрузки на миниатюре работы<br>
- При предварительном просмотре работы, использовании просмотрщика изображений или просмотре оригинального изображения нажмите кнопку загрузки или используйте горячую клавишу для создания задачи загрузки<br>
- Сканирование вручную выбранных работ или изображений<br>
- Нажатие кнопки быстрой загрузки в правой части страницы работы<br>
...
- Сканирование списка тегов`,
  ...
  _预览搜索结果说明: [
    ...
    You can set the maximum number of preview results and how many works to show per page. Pagination reduces the page's memory use and rendering load.`,
...
プレビュー結果の上限と、1ページに表示する作品数を設定できます。ページ分割により、メモリ使用量と描画負荷を軽減できます。`,
...
미리보기 결과의 최대 개수와 페이지당 표시할 작품 수를 설정할 수 있습니다. 페이지를 나누면 메모리 사용량과 렌더링 부담을 줄일 수 있습니다.`,
...
Можно задать максимальное количество результатов предпросмотра и число работ на странице. Разбиение на страницы снижает использование памяти и нагрузку на отображение.`,
  ],
...
Если на странице показано больше 0 работ, возможно, Pixiv отклонил эту попытку сканирования. Подождите несколько минут и попробуйте снова.`,
  ...
  _查看作品原图: [
    `查看作品原图`,
    `查看作品原圖`,
    `View the original image of the work`,
    `作品の原図を表示`,
    `작품 원본 이미지 보기`,
    `Просмотреть оригинальное изображение работы`,
  ],
  _查看作品大图时的快捷键: [
...
<span class="blue">V</span>, <span class="blue">Esc</span> Закрыть предпросмотр. Также предпросмотр можно закрыть, щёлкнув левой кнопкой мыши по нему<br>`,
  ...
  _自动导出日志: [
    `自动<span class="key">导出</span>日志`,
    `自動<span class="key">匯出</span>日誌`,
    `Automatic <span class="key">log export</span>`,
    `ログの<span class="key">自動エクスポート</span>`,
    `<span class="key">자동 로그 내보내기</span>`,
    `Автоматический <span class="key">экспорт журнала</span>`,
  ],
  _自动导出日志的说明: [
    `如果你希望下载器在抓取或下载完成后自动导出日志，可以启用此功能。<br>
<br>
在“日志类型”子选项里，默认只选择了“错误”类型，所以只会保存含有错误信息的日志。<br>
如果你想保存所有日志，需要同时选择“正常”类型。<br>
<br>
...
Чтобы сохранить все журналы, также выберите тип «Обычный».<br>
<br>
...
Внизу области журнала также есть кнопка для ручного экспорта журналов. Эти два способа экспорта журналов независимы. Если вы хотите сохранять журналы лишь изредка, можно использовать только ручной экспорт.`,
  ...
  _正在加载关注用户列表: [
    `正在加载关注用户列表`,
    `正在載入關注使用者列表`,
    `Loading the list of following users`,
    `フォローしているユーザーのリストを読み込んでいます`,
    `팔로우하는 사용자 목록을 로드하고 있습니다`,
    `Загрузка списка следующих пользователей`,
  ],
  _正在加载关注用户列表_该数据会保存在本地: [
...
Если вы хотите изменить эти горячие клавиши, это можно сделать в разделе загрузчика «Общие» - «Способ работы» - «Пользовательские горячие клавиши».`,
  ...
  _自定义快捷键: [
    `自定义<span class="key">快捷键</span>`,
    `自訂<span class="key">快捷鍵</span>`,
    `Customize <span class="key">hotkeys</span>`,
    `<span class="key">ショートカットキー</span>をカスタマイズ`,
    `<span class="key">단축키</span> 사용자 지정`,
    `Настроить <span class="key">горячие клавиши</span>`,
  ],
  _自定义快捷键的说明: [
    `你可以使用这些按键来作为快捷键：<br>
...
- Вы можете изменять горячие клавиши первого уровня загрузчика; горячие клавиши второго уровня (например, используемые только при «предпросмотре работы») не поддерживаются.`,
  ...
  _允许预览图遮挡缩略图: [
    ...
    `プレビュー画像がサムネイルを覆うことを許可する`,
    `미리보기 이미지가 썸네일을 덮도록 허용`,
    `Разрешить предварительному изображению перекрывать миниатюру`,
  ],
  _允许预览图遮挡缩略图的帮助: [
    `当你预览横图时，预览图的宽度经常是 1200px，但是缩略图左侧或右侧的空间可能不足 1200px，此时：<br>
- 如果启用此开关，那么预览图会尽量保持原始大小（不缩小），但可能会遮挡住缩略图。提示：此时你可以点击缩略图来使它消失。<br>
- 如果禁用此开关，那么预览图通常会显示的更小，以避免遮挡住缩略图。`,
    `當你預覽橫圖時，預覽圖的寬度經常是 1200px，但是縮略圖左側或右側的空間可能不足 1200px，此時：<br>
- 如果啟用此開關，那麼預覽圖會盡量保持原始大小（不縮小），但可能會遮擋住縮略圖。提示：此時你可以點擊縮略圖來使它消失。<br>
- 如果停用此開關，那麼預覽圖通常會顯示得更小，以避免遮擋住縮略圖。`,
    `When you preview a landscape image, the preview image is often 1200px wide, but the space on the left or right side of the thumbnail may be less than 1200px. In this case:<br>
- If this option is enabled, the preview image will try to keep its original size (not be scaled down), but it may cover the thumbnail. Tip: you can click the thumbnail to make it disappear.<br>
- If this option is disabled, the preview image will usually be displayed smaller to avoid covering the thumbnail.`,
    `横向きの画像をプレビューするとき、プレビュー画像の幅は 1200px であることがよくありますが、サムネイルの左側または右側のスペースが 1200px に満たない場合があります。この場合：<br>
- このスイッチを有効にすると、プレビュー画像は元のサイズをできるだけ維持します（縮小しません）が、サムネイルを覆う可能性があります。ヒント：このときサムネイルをクリックすると消えます。<br>
- このスイッチを無効にすると、プレビュー画像は通常、サムネイルを覆わないように小さく表示されます。`,
    `가로 이미지를 미리 볼 때 미리보기 이미지의 너비는 보통 1200px이지만, 썸네일 왼쪽 또는 오른쪽의 공간이 1200px 미만일 수 있습니다. 이 경우:<br>
- 이 스위치를 켜면 미리보기 이미지가 원본 크기를 최대한 유지하지만(축소하지 않음), 썸네일을 가릴 수 있습니다. 힌트: 이때 썸네일을 클릭하면 사라집니다.<br>
- 이 스위치를 끄면 미리보기 이미지가 보통 더 작게 표시되어 썸네일을 가리지 않습니다.`,
...
Одна из возможных причин: ваша учетная запись заблокирована Pixiv.`,
  ...
  _提示当前页面上没有可以用于手动删除的元素: [
    `当你启用“预览搜索页面的筛选结果”功能之后，下载器会把抓取到的作品显示在页面上，供用户查看。<br>
    现在页面上没有下载器添加的作品，所以无法使用手动删除作品功能。<br>
    这可能是因为你没有启用“预览搜索页面的筛选结果”功能，或者目前没有任何抓取结果。<br>
    <br>
    作为替代方案，你可以使用“手动排除作品”按钮来排除不需要的作品。`,
    `啟用「預覽搜尋頁面的篩選結果」功能後，下載器會將抓取到的作品顯示在頁面上，供你查看。<br>
    目前頁面上沒有下載器添加的作品，因此無法使用「手動刪除作品」功能。<br>
    這可能是因為你沒有啟用「預覽搜尋頁面的篩選結果」功能，或目前沒有任何抓取結果。<br>
    <br>
    你也可以改用「手動排除作品」按鈕，排除不需要的作品。`,
    `When you enable "Preview filter results on search page", the downloader displays crawled works on the page so you can review them.<br>
    There are currently no works added by the downloader on the page, so you can't use "Manually delete works".<br>
    This may be because "Preview filter results on search page" is disabled or there are no crawl results yet.<br>
    <br>
    Alternatively, you can use the "Manually exclude" button to exclude works you don't want.`,
    `「検索ページのフィルタ結果をプレビュー」機能を有効にすると、ダウンローダーがクロールした作品がページ上に表示され、確認できます。<br>
    現在、ページ上にダウンローダーが追加した作品がないため、「作品を手動で削除する」機能は使えません。<br>
    「検索ページのフィルタ結果をプレビュー」機能が有効になっていないか、クロール結果がまだない可能性があります。<br>
    <br>
    代わりに、「手動で作品を除外」ボタンを使って、不要な作品を除外できます。`,
    `"검색 페이지 필터 결과 미리보기" 기능을 활성화하면 다운로더가 긁어온 작품을 페이지에 표시하여 확인할 수 있습니다.<br>
    현재 페이지에 다운로더가 추가한 작품이 없어 "수동으로 작품 지우기" 기능을 사용할 수 없습니다.<br>
    "검색 페이지 필터 결과 미리보기" 기능이 활성화되어 있지 않거나 아직 긁어온 결과가 없기 때문일 수 있습니다.<br>
    <br>
    대신 "수동 제외" 버튼을 사용해 원하지 않는 작품을 제외할 수 있습니다.`,
    `Когда вы включаете функцию «Предварительный просмотр результатов фильтрации на странице поиска», загрузчик показывает просканированные работы на странице, чтобы вы могли их просмотреть.<br>
    Сейчас на странице нет работ, добавленных загрузчиком, поэтому функцию «Вручную удалить работу» использовать нельзя.<br>
    Возможно, функция «Предварительный просмотр результатов фильтрации на странице поиска» не включена или результатов сканирования пока нет.<br>
    <br>
