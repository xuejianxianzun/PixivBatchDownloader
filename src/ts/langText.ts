...
Одна из возможных причин: ваша учетная запись заблокирована Pixiv.`,
  ...
  _EPUB文字方向: [
    `EPUB 文字方向`,
    `EPUB 文字方向`,
    `EPUB writing direction`,
    `EPUB 文字方向`,
    `EPUB 텍스트 방향`,
    `Направление текста EPUB`,
  ],
  _EPUB文字方向横排: [
    `横排`,
    `橫排`,
  ...
  _EPUB文字方向的说明: [
    `设置 EPUB 文件的文字方向。纵排适合日语等 CJK 小说，会将 EPUB 阅读器的翻页方向设置为从右向左。`,
    `設定 EPUB 檔案的文字方向。縱排適合日語等 CJK 小說，會將 EPUB 閱讀器的翻頁方向設定為從右向左。`,
    `Set the text direction of the EPUB file. Vertical is suitable for CJK novels such as Japanese, and sets the page progression direction to right-to-left.`,
    `EPUB ファイルの文字方向を設定します。縦組みは日本語などの CJK 小説に適しており、ページ送り方向を右から左に設定します。`,
    `EPUB 파일의 텍스트 방향을 설정합니다. 세로쓰기는 일본어 등 CJK 소설에 적합하며, 페이지 진행 방향이 오른쪽에서 왼쪽으로 설정됩니다.`,
    `Установите направление текста в файле EPUB. Вертикальное подходит для CJK-романов, например японских, и устанавливает направление перелистывания страниц справа налево.`,
  ],
  _指定EPUB语言: [
    `指定 EPUB 语言`,
    `指定 EPUB 語言`,
    `Specify EPUB language`,
    `EPUB の言語を指定する`,
    `EPUB 언어 지정`,
    `Указать язык EPUB`,
  ],
  _指定EPUB语言的说明: [
    `开启后，EPUB 文件的 dc:language 和 xml:lang 属性将使用指定的语言标签，而不是下载器界面的语言。`,
    `開啟後，EPUB 檔案的 dc:language 和 xml:lang 屬性將使用指定的語言標籤，而非下載器介面的語言。`,
    `When enabled, the dc:language and xml:lang attributes in the EPUB will use the specified language tag instead of the downloader UI language.`,
    `有効にすると、EPUB の dc:language および xml:lang 属性に、ダウンローダーの UI 言語ではなく指定した言語タグが使用されます。`,
    `활성화하면 EPUB의 dc:language 및 xml:lang 속성에 다운로더 UI 언어 대신 지정한 언어 태그가 사용됩니다.`,
    `При включении атрибуты dc:language и xml:lang в EPUB будут использовать указанный языковой тег вместо языка интерфейса загрузчика.`,
  ],
  _从Pixiv获取语言: [
    `从 Pixiv 获取`,
    `從 Pixiv 獲取`,
