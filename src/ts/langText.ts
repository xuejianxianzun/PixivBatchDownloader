...
И так далее. <br>`,
  ...
  _抓取结果为零: [
    ...
    'Извлечение завершено, но не найдены работы, соответствующие критериям фильтра.<br>Пожалуйста, проверьте настройки, связанные со сканированием.',
  ],
  _当前任务尚未完成: [
    '当前任务尚未完成',
    '目前工作尚未完成',
  ...
  _作品页状态码429: [
    ...
    'Код ошибки: 429 (Слишком много запросов). Загрузчик будет повторять извлечение.',
  ],
  _错误代码: [
    '错误代码：',
    '錯誤程式碼：',
  ...
  _作品页状态码500: [
    ...
    'Pixiv отказывается возвращать данные (500). Загрузчик будет повторять извлечение.',
  ],
  _正在抓取: [
    '正在抓取，请等待……',
    '擷取中，請稍後……',
...
Chrono Download Manager, Image Downloader и т. д.`,
  ...
  _正在恢复抓取结果: [
    ...
    'クロール結果を再開しています',
    '긁어오기 결과 복구 중',
    'Восстановление результатов извлечения',
  ],
  _已恢复抓取结果: [
    '已恢复抓取结果',
    '已還原擷取結果',
    ...
    'クロール結果を再開しました',
    '긁어오기 결과가 복구되었습니다',
    'Результаты извлечения восстановлены',
  ],
  _清空已保存的抓取结果: [
    '清空已保存的抓取结果',
    '清除已儲存的擷取結果',
    ...
    'セーブしたクロール結果をクリアします',
    '저장된 긁어오기 결과 비우기',
    'Очистить сохраненные результаты извлечения',
  ],
  _数据清除完毕: [
    '数据清除完毕',
    '資料清除完畢',
  ...
  _不抓取多图作品的最后一张图片: [
    ...
    'Не сканировать <span class="key">последние изображения</span> в много картинных работах',
  ],
  _下载小说的封面图片: [
