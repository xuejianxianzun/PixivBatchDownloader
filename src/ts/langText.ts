...
Формат JSON — это внутренние данные загрузчика, сохраняющие гораздо больше информации.`,
  ...
  _只有一个抓取结果时不建立文件夹的提示: [
    `只有一个抓取结果时生效。<br>注意：如果一个作品产生了多个抓取结果（文件），就仍然会创建文件夹。只有当抓取结果为 1 时才不会建立文件夹。`,
    `只有一個抓取結果時生效。<br>注意：如果一個作品產生了多個抓取結果（檔案），就仍然會建立資料夾。只有當抓取結果為 1 時才不會建立資料夾。`,
    `Takes effect only when there is a single crawl result.<br>Note: If a work produces multiple crawl results (files), a folder will still be created. A folder will not be created only when there is exactly 1 crawl result.`,
    `クロール結果が1つの場合のみ有効になります。<br>注意：1つの作品が複数のクロール結果（ファイル）を生成した場合、フォルダは引き続き作成されます。クロール結果がちょうど1つの場合のみフォルダは作成されません。`,
    `크롤링 결과가 하나일 때만 적용됩니다.<br>주의: 한 작품에서 여러 크롤링 결과(파일)가 생성되면 폴더가 여전히 생성됩니다. 크롤링 결과가 정확히 1개일 때만 폴더가 생성되지 않습니다.`,
    `Действует только при единственном результате краулинга.<br>Примечание: Если работа даёт несколько результатов краулинга (файлов), папка всё равно будет создана. Папка не создаётся только когда результат краулинга ровно 1.`,
  ],
  _搜索页面页数限制: [
