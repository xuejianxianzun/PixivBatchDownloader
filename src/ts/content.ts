...
class PageInfoClass {
  ...
  // 获取当前页面的一些信息，用于文件名中
  getPageInfo(type: number): PageInfo {
...
class GetOption extends Config {
  ...
  getNotNeedTag() {
    ...
    if (tempNotNeedTag === '') {
      ...
      Log.add(
        lang.transl('_设置了排除tag之后的提示') + this.notNeedTag.join(','),
        1
      )
    }
  }
  ...
  getNeedTag() {
    ...
    if (tempNeedTag === '') {
      ...
      Log.add(
        lang.transl('_设置了必须tag之后的提示') + this.needTag.join(','),
        1
      )
    }
  }
  ...
  checkSetBmk() {
    ...
    if (checkResult.result) {
      ...
      Log.add(
        lang.transl('_设置了筛选收藏数之后的提示文字') + this.filterBmk,
        1
      )
    }
...
*/
  ...
  xzTip(this: HTMLElement, arg: XzTipArg) {
    ...
    if (arg.type === 1) {
      this.xzTipEl.innerHTML = tipText
  ...
  private addDownloadPanel() {
    ...
    this.centerBtnWrap = document.querySelector(
      '.centerWrap_btns_free'
    )! as HTMLDivElement
    this.xzForm = document.querySelector('.xzForm')! as XzForm
    this.pauseBtn = document.querySelector('.pauseDownload')! as HTMLDivElement
    this.stopBtn = document.querySelector('.stopDownload')! as HTMLDivElement
  }
  ...
  private downloadPanelEvents() {
    ...
    const centerInputs: NodeListOf<HTMLInputElement> = this.xzForm.querySelectorAll(
      'input[type=text]'
    )
    ...
    // 把下拉框的选择项插入到文本框里
    this.insertValueToInput(this.xzForm.pageInfoSelect, this.xzForm.fileNameRule)
    this.insertValueToInput(this.xzForm.fileNameSelect, this.xzForm.fileNameRule)
  }
...
class Form{
  ...
  // 把获取到的页面信息添加到下拉选项里
  public setPageInfoSelector(info: PageInfo) {
    const pageInfoSelect = this.xzForm.pageInfoSelect
  ...
  private initFormOption() {
    ...
    // 设置作品张数
    const setPNoInput = this.xzForm.setPNo
    ...
    setPNoInput.addEventListener('change', function(this: HTMLInputElement) {
      if (parseInt(this.value) >= 0) {
        that.saveXzSetting('imgNumberPerWork', this.value)
      }
    })
    ...
    for (let index = 0; index < this.xzSetting.notdownType.length; index++) {
      ...
      ;(this.xzForm[name] as HTMLInputElement).checked = false
    }

    // 保存排除类型
    for (let index = 0; index < 3; index++) {
      let name = 'setWorkType' + index.toString()
      ;(this.xzForm[name] as HTMLInputElement).addEventListener('click', () => {
        this.saveXzSetting('notdownType', getNotDownType())
      })
    }

    // 设置动图格式选项
    this.xzForm.ugoiraSaveAs.value = this.xzSetting.ugoiraSaveAs || 'webm'

    // 保存动图格式选项
    for (const input of this.xzForm.ugoiraSaveAs) {
      input.addEventListener('click', function(this: HTMLInputElement) {
        that.saveXzSetting('ugoiraSaveAs', this.value)
      })
    }

    // 设置必须的 tag
    const setTagNeedInput = this.xzForm.setTagNeed
    setTagNeedInput.value = this.xzSetting.needTag.join(',')

    // 保存必须的 tag设置
    setTagNeedInput.addEventListener('change', function(
      this: HTMLInputElement
    ) {
      that.saveXzSetting('needTag', this.value)
    })

    // 设置排除的 tag
    const setTagNotNeedInput = this.xzForm.setTagNotNeed
    setTagNotNeedInput.value = this.xzSetting.notNeedTag.join(',')

    // 保存排除的 tag设置
    setTagNotNeedInput.addEventListener('change', function(
      this: HTMLInputElement
    ) {
      that.saveXzSetting('notNeedTag', this.value)
    })

    // 设置是否显示封面
    const setDisplayCoverInput = this.xzForm.setDisplayCover
    ...
    setDisplayCoverInput.addEventListener('click', function(
      ...
      that.saveXzSetting('displayCover', this.checked)
    })
    ...
    // 设置快速下载
    const setQuietDownloadInput = this.xzForm.setQuietDownload
    ...
    setQuietDownloadInput.addEventListener('click', function(
      ...
      that.saveXzSetting('quietDownload', this.checked)
    })

    // 设置下载线程
    const setThreadInput = this.xzForm.setThread
    ...
    setThreadInput.addEventListener('change', function(this: HTMLInputElement) {
      if (parseInt(this.value) > 0 && parseInt(this.value) <= 5) {
        that.saveXzSetting('downloadThread', this.value)
      }
    })

    // 设置文件命名规则
    const fileNameRuleInput = this.xzForm.fileNameRule
    ...
    fileNameRuleInput.addEventListener('change', function(
      ...
      if (this.value !== '') {
        that.saveXzSetting('userSetName', this.value)
      } else {
        // 把下拉框恢复默认值
        that.xzForm.fileNameSelect.value = (that.xzForm.fileNameSelect
          .children[0] as HTMLOptionElement).value
      }
    })

    // 是否添加字段名称
    const setTagNameToFileNameInput = this.xzForm.setTagNameToFileName
    ...
    setTagNameToFileNameInput.addEventListener('click', function(
      ...
      that.saveXzSetting('tagNameToFileName', this.checked)
    })
  }
...
class Log {
  ...
  static checkElement() {
    ...
    if (test === null) {
      ...
      this.insertToHead(this.logArea)
    }
  }
...
class APIs {
  ...
  static getUserInfo(id: string = '') {
    fetch(
      `https://www.pixiv.net/ajax/user/${id || this.getUserId()}/profile/top`,
      {
        method: 'get',
...
*/
  ...
  outputNowResult() {
    Log.add(
      lang.transl('_调整完毕', visibleList().length.toString()),
      0,
      2,
      false
    )
  ...
  getListPage() {
    ...
    if (this.downRelated) {
      ...
      Log.add(
        lang.transl('_排行榜任务完成', this.illustUrlList.length.toString())
      )
      this.getListUrlFinished()
      return false
    } else {
      ...
      .then((data: string) => {
        ...
        if (this.downRelated) {
          ...
          Log.add(
            lang.transl(
              '_相关作品抓取完毕',
              this.illustUrlList.length.toString()
            )
          )
          this.getListUrlFinished()
        } else if (pageType === 5) {
  ...
  readyGetListPage() {
    ...
    // 是否是 tag 模式
    this.hasTag = !!getQuery(locUrl, 'tag')
    ...
    if (locUrl.includes('member.php?id=')) {
      ...
      apiUrl = `https://www.pixiv.net/ajax/user/${getUserId()}/illusts/bookmarks?tag=${nowTag}&offset=${this.offsetNumber}&limit=${onceRequest}&rest=${restMode}`
    } else {
      // 不进行抓取
      this.allowWork = true
      return false
      ...
      .then((data: Type2ListDataNoTag & Type2ListDataHaveTag) => {
        ...
        if (!this.hasTag) {
          ...
          if (this.type2ListType === 0) {
            ...
            // 插画列表页，包含动图
            this.type2IdList = this.type2IdList.concat(
          ...
          if (this.type2ListType !== 3) {
            ...
            if (
              works.length === 0 ||
              this.type2IdList.length >= this.requsetNumber
            ) {
        ...
        if (this.type2IdList.length > 0) {
          if (
            ...
            this.type2IdList.splice(
              this.type2IdList.length - this.offsetNumber,
          ...
          if (this.type2ListType !== 3 || bmkGetEnd) {
            ...
            this.offsetNumber += onceRequest // 每次增加偏移量，并获取之后固定数量
            url = url.replace(/offset=\d*\d?/, `offset=${this.offsetNumber}`)
  ...
  outputImgNum() {
    Log.add(
      lang.transl('_抓取图片网址的数量', this.imgInfo.length.toString()),
      -1,
      1,
      false
    )
  ...
  pageType2() {
    ...
    if (columnTitle) {
      const downRecmdBtn = addCenterButton(
        Colors.blue,
        lang.transl('_抓取推荐作品'),
