    ...
    async addBookmarkAll() {
        return new Promise(async (resolve) => {
            ...
            for (const data of this.bookmarKData) {
                ...
                await _API__WEBPACK_IMPORTED_MODULE_0__["API"].addBookmark(this.type, data.id, _setting_Settings__WEBPACK_IMPORTED_MODULE_5__["settings"].quickBookmarks ? data.tags : [], data.restrict, this.token);
                index++;
            }
            resolve();
        });
...
class Support {
    ...
    async checkNew() {
        ...
        if (!lastTime || new Date().getTime() - parseInt(lastTime) > interval) {
            // 获取最新的 releases 信息
            const latest = await fetch(_Config__WEBPACK_IMPORTED_MODULE_2__["default"].latestReleaseAPI);
