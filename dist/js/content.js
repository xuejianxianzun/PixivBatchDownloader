        ...
        for (let index = 0; index < this.allTabTitle.length; index++) {
            eventList.forEach((eventName) => {
                this.allTabTitle[index].addEventListener(eventName, () => {
                    ...
                    if (eventName === 'mouseenter' && !_setting_Settings__WEBPACK_IMPORTED_MODULE_8__["settings"].mouseEnterSwitchTabbar) {
                        return;
                    }
...
class FileName {
    ...
    appendFolder(fullPath, folderName) {
        ...
        allPart.splice(allPart.length - 1, 0, _utils_Utils__WEBPACK_IMPORTED_MODULE_4__["Utils"].replaceUnsafeStr(folderName));
        return allPart.join('/');
    }
    getFileName(data) {
        // 1 使命名规则合法化
        ...
        if (result !== _setting_Settings__WEBPACK_IMPORTED_MODULE_0__["settings"].userSetName) {
            Object(_setting_Settings__WEBPACK_IMPORTED_MODULE_0__["setSetting"])('userSetName', result);
        }
        // 2 配置所有命名标记
        const cfg = {
            '{p_title}': {
        ...
        if (_setting_Settings__WEBPACK_IMPORTED_MODULE_0__["settings"].r18Folder && (data.xRestrict === 1 || data.xRestrict === 2)) {
            result = this.appendFolder(result, _setting_Settings__WEBPACK_IMPORTED_MODULE_0__["settings"].r18FolderName);
        }
        // 为作品建立单独的文件夹
...
class DownloadControl {
    ...
    /**为了防止文件名重复，命名规则里一定要包含 {id} 或者 {id_num}{p_num} */
    checkNamingRule() {
        if (_setting_Settings__WEBPACK_IMPORTED_MODULE_6__["settings"].userSetName.includes('{id}') || (_setting_Settings__WEBPACK_IMPORTED_MODULE_6__["settings"].userSetName.includes('{id_num}') && _setting_Settings__WEBPACK_IMPORTED_MODULE_6__["settings"].userSetName.includes('{p_num}'))) {
            return true;
        }
        else {
            _MsgBox__WEBPACK_IMPORTED_MODULE_18__["msgBox"].error(_Lang__WEBPACK_IMPORTED_MODULE_4__["lang"].transl('_命名规则一定要包含id'));
            _EVT__WEBPACK_IMPORTED_MODULE_0__["EVT"].fire(_EVT__WEBPACK_IMPORTED_MODULE_0__["EVT"].list.openCenterPanel);
            _EVT__WEBPACK_IMPORTED_MODULE_0__["EVT"].fire(_EVT__WEBPACK_IMPORTED_MODULE_0__["EVT"].list.downloadCancel);
            return false;
        }
    }
    createDownloadArea() {
