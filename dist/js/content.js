...
class FileName {
    ...
    createPNum(data) {
        var _a;
        const index = (_a = data.index) !== null && _a !== void 0 ? _a : _Tools__WEBPACK_IMPORTED_MODULE_5__["Tools"].getResultIndex(data);
        // 处理第一张图不带序号的情况
        ...
        if (data.type === 0 || data.type === 1) {
            ...
            // 根据需要在前面填充 0
            return _setting_Settings__WEBPACK_IMPORTED_MODULE_0__["settings"].zeroPadding ? p.padStart(_setting_Settings__WEBPACK_IMPORTED_MODULE_0__["settings"].zeroPaddingLength, '0') : p;
        }
        else {
            // 其他类型没有编号，返回空字符串
            return '';
    ...
    createId(data) {
        ...
        if (!_setting_Settings__WEBPACK_IMPORTED_MODULE_0__["settings"].zeroPadding) {
            ...
            // 只有插画和漫画有编号
            if (data.type === 0 || data.type === 1) {
                return data.idNum + '_p' + index.toString().padStart(_setting_Settings__WEBPACK_IMPORTED_MODULE_0__["settings"].zeroPaddingLength, '0');
            }
            else {
    ...
    getFileName(data) {
        ...
        for (const [key, val] of Object.entries(cfg)) {
            if (result.includes(key)) {
                // 替换空值
                let temp = val.value === undefined || val.value === null ? '' : val.value;
                // 如果这个值不是字符串则转换为字符串
                temp = typeof temp !== 'string' ? temp.toString() : temp;
                // 替换值里不可以作为文件名的特殊字符
                if (!val.safe) {
                    temp = _utils_Utils__WEBPACK_IMPORTED_MODULE_4__["Utils"].replaceUnsafeStr(temp);
                }
                // 添加标记名称
...
class DownloadControl {
    ...
    /**为了防止文件名重复，命名规则里一定要包含 {id} 或者 {id_num}{p_num} */
    checkNamingRule() {
        if (_setting_Settings__WEBPACK_IMPORTED_MODULE_6__["settings"].userSetName.includes('{id}') ||
            (_setting_Settings__WEBPACK_IMPORTED_MODULE_6__["settings"].userSetName.includes('{id_num}') &&
                _setting_Settings__WEBPACK_IMPORTED_MODULE_6__["settings"].userSetName.includes('{p_num}'))) {
            return true;
        }
        else {
...
class OutputPanel {
    ...
    output(data) {
        ...
        if (_store_Store__WEBPACK_IMPORTED_MODULE_2__["store"].result.length > _config_Config__WEBPACK_IMPORTED_MODULE_4__["Config"].outputMax) {
            ...
            data.content = _Lang__WEBPACK_IMPORTED_MODULE_1__["lang"].transl('_输出内容太多已经为你保存到文件');
        }
        else {
...
class Settings {
    ...
    handleUserSetName(val) {
        ...
        let result = val || '{id}';
        // 替换命名规则里可能存在的非法字符
        result = _utils_Utils__WEBPACK_IMPORTED_MODULE_1__["Utils"].replaceUnsafeStr(result);
