...
function getNotNeedTag() {
    ...
    if (tempNotNeedTag === '') {
        ...
        addOutputInfo('<br>' + xzlt('_设置了排除tag之后的提示') + notNeedTag.join(','));
    }
}
// 获取必须包含的tag
function getNeedTag() {
...
function getRatioSetting() {
    ...
    if (ratioType === '1') {
        ...
        addOutputInfo('<br>' +
            xzlt('_设置了宽高比之后的提示', xzlt('_输入宽高比') + xzForm.userRatio.value));
    }
    return true;
}
// 检查作品是否符合宽高比条件
...
function startGet() {
    ...
    if (pageType === 1) {
        ...
        const result = checkWantPageInput(xzlt('_checkWantPageRule1Arg2'), '', xzlt('_checkWantPageRule1Arg7'));
        if (!result) {
            return false;
        }
        if (wantPage === -1) {
        ...
        addOutputInfo(xzlt('_tag搜索任务开始', xzForm.setFavNum.value, wantPage.toString()));
    ...
    fetch(url)
        ...
        if (downRelated) {
            ...
            outputArea.innerHTML =
                nowTips +
                    '<br>' +
                    xzlt('_tag搜索页已抓取多少页', tagPageFinished.toString(), wantPage.toString(), (startpageNo + listPageFinished - 1).toString());
            // 每抓取完一页，判断任务状态
            ...
            if (complete || listPageFinished === partNumber) {
                if (illustUrlList.length === 0) {
                    ...
                    addOutputInfo('<br>' + xzlt('_排行榜任务完成', illustUrlList.length.toString()));
                    getListUrlFinished();
                }
            }
            else {
...
function crawFinished() {
    ...
    if (testSuffixFinished) {
        ...
        addOutputInfo('<br>' + xzlt('_获取图片网址完毕', imgInfo.length.toString()) + '<br>');
        if (imgInfo.length === 0) {
...
function downloadFile(downloadBarNo) {
    ...
    xhr.addEventListener('loadend', async function () {
        ...
        if (xhr.status !== 200) {
            ...
            if (xhr.status === 404) {
                // 输出提示信息
                addOutputInfo(`<span style="color:#f00">${xzlt('_file404', thisImgInfo.id)}</span><br>`);
