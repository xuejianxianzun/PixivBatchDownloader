...
const formHtml = `<form class="settingForm">
  ...
  <div class="tabsContnet">
    ...
    <div class="con">
      ...
      <span class="has_tip settingNameStyle1" data-tip="${_Lang__WEBPACK_IMPORTED_MODULE_0__["lang"].transl('_在小说内保存元数据提示')}">${_Lang__WEBPACK_IMPORTED_MODULE_0__["lang"].transl('_在小说内保存元数据')}<span class="gray1"> ? </span></span>
...
const langText = {
    ...
    _小说保存格式: ['小说保存格式'],
    _在小说内保存元数据: ['在小说内保存元数据'],
    _在小说内保存元数据提示: ['把作者、网址等信息保存到文件里'],
    _把系列小说合并成一个文件: ['把系列小说合并成一个文件'],
    _把系列小说合并成一个文件提示: ['只在系列小说页面里生效'],
};
...
class SaveNovelData {
    async save(data) {
        ...
        if (await _Filter__WEBPACK_IMPORTED_MODULE_0__["filter"].check(filterOpt)) {
            ...
            const ext = _Settings__WEBPACK_IMPORTED_MODULE_2__["form"].novelSaveAs.value;
            let metaArr = [];
            let meta = '';
