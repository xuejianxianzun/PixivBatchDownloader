...
let logArea = document.createElement('div'); // 输出信息的区域
...
let hasTag = false; // pageType 2 里，是否带 tag
let type2ListType; // pageType 2 里的页面类型，都是列表页
...
let addTagList = []; // 需要添加 tag 的作品列表
let addTagBtn; // 给未分类作品添加 tag 的按钮
...
let wantPage = 0; // 要抓取几页
let quickDownload = false; // 快速下载当前作品，这个只在作品页内直接下载时使用
...
let debut = false; // 只下载首次登场的作品
let listIsNewMode = false; // 列表页加载模式是否是新版
...
const tagSearchMultipleSelector = '._3b8AXEx'; // 作品选择器
...
1 抓取旧作品
*/
let downDirection = 0; // 抓取方向，在作品页内指示抓取新作品还是旧作品
...
async function readyAddTag(loop = 0) {
    const offset = loop * 100; // 一次请求只能获取 100 个，所以可能有多次请求，要计算偏移量
    // 配置 url
    const showUrl = `https://www.pixiv.net/ajax/user/${getUserId()}/illusts/bookmarks?tag=${encodeURI('未分類')}&offset=${offset}&limit=100&rest=show&rdm=${Math.random()}`; // 公开的未分类收藏
...
// 给未分类作品添加 tag
async function addTag(index, addList, tt) {
...
2 error 红色
*/
function addLog(str, level = -1, br = 1, addMode = true) {
...
function checkNotDownType() {
    ...
    // 排除了至少一种时，显示提示
    if (notdownType.includes('0') ||
        notdownType.includes('1') ||
        notdownType.includes('2')) {
        addLog(xzlt('_checkNotdownTypeResult3Html') +
            notdownType
                .replace('0', xzlt('_插画'))
                .replace('1', xzlt('_漫画'))
                .replace('2', xzlt('_动图')), 1);
    }
}
// 检查是否设置了作品张数限制
...
function getNowPageNo() {
    ...
    if (document.querySelector('.page-list .current')) {
        startpageNo = parseInt(document.querySelector('.page-list .current').textContent);
    }
    else {
        // 否则认为只有1页
        startpageNo = 1;
...
function tagSearchPageFinished() {
    ...
    addLog(xzlt('_当前作品张数', document.querySelectorAll(tagSearchListSelector).length.toString()), -1, 2);
    listSort();
    changeTitle('→');
}
// 获取 tag 搜索列表里的可见作品
...
function startGet() {
    ...
    if (pageType === 1) {
        ...
        if (quickDownload) {
            ...
            let result = false;
            if (!downRelated) {
                result = checkWantPageInput(xzlt('_checkWantPageRule1Arg2'), xzlt('_checkWantPageRule1Arg3'), xzlt('_checkWantPageRule1Arg4'));
            }
            else {
                // 相关作品的提示
                result = checkWantPageInput(xzlt('_checkWantPageRule1Arg2'), xzlt('_checkWantPageRule1Arg9'), xzlt('_checkWantPageRule1Arg10'));
            }
            if (!result) {
                return false;
            }
...
function addIllustUrlList(arr) {
    arr.forEach(data => {
        illustUrlList.push('https://www.pixiv.net/member_illust.php?mode=medium&illust_id=' + data);
    });
}
// 获取作品列表页
    ...
    fetch(url)
        ...
        if (downRelated) {
            ...
            for (const data of thisOneData) {
                ...
                if (!checkSetWhok(tureWidth, tureHeight) ||
            ...
            addLog(xzlt('_tag搜索页已抓取多少页', tagPageFinished.toString(), wantPage.toString(), (startpageNo + listPageFinished - 1).toString()), -1, 1, false);
            // 每抓取完一页，判断任务状态
            ...
            for (const data of contents) {
                ...
                // 检查宽高设置和宽高比设置
                if (!checkSetWhok(data.width, data.height) ||
            ...
            addLog(xzlt('_列表页抓取进度', listPageFinished.toString()), -1, 1, false);
            // 判断任务状态
            // 如果没有下一页的按钮或者抓取完指定页面
...
function readyGetListPage() {
    ...
    if (locUrl.includes('member.php?id=')) {
        ...
        if (getQuery(locUrl, 'type') === 'illust') {
            ...
            // 带 tag
            if (hasTag) {
                apiUrl = `https://www.pixiv.net/ajax/user/${getUserId()}/illusts/tag?tag=${getQuery(locUrl, 'tag')}&offset=${offsetNumber}&limit=${requsetNumber}`;
            }
        }
            ...
            // 带 tag
            if (hasTag) {
                apiUrl = `https://www.pixiv.net/ajax/user/${getUserId()}/manga/tag?tag=${getQuery(locUrl, 'tag')}&offset=${offsetNumber}&limit=${requsetNumber}`;
            }
        }
        else if (hasTag) {
            // url 里没有插画也没有漫画，但是有 tag，则是在资料页首页点击了 tag，需要同时获取插画和漫画
            type2ListType = 4;
            apiUrl = `https://www.pixiv.net/ajax/user/${getUserId()}/illustmanga/tag?tag=${getQuery(locUrl, 'tag')}&offset=${offsetNumber}&limit=${requsetNumber}`;
        }
    }
    ...
    fetch(url, {
        ...
        if (type2IdList.length > 0) {
            if (type2ListType === 0 ||
                ...
                // 删除后面的 id（删除不需要的近期作品）
                type2IdList.splice(type2IdList.length - offsetNumber, type2IdList.length);
            }
...
// 当因为网络问题无法获取作品数据时，重试
function reTryGetIllustData(url) {
    setTimeout(() => {
        getIllustData(url);
    }, 2000);
}
// 获取作品的数据
async function getIllustData(url) {
    // url 参数为完整的作品页 url，如：
    ...
    try {
        ...
        if (response.ok) {
            ...
            // 检查排除类型设置，这里取反
            const notdownTypeResult = !notdownType.includes(jsInfo.illustType.toString());
            ...
            if (pageType === 1 && !downRelated) {
                ...
                if (wantPage === -1 || wantPage > 0) {
                    ...
                    if (nextId) {
                        getIllustData('https://www.pixiv.net/member_illust.php?mode=medium&illust_id=' +
                            nextId);
                    }
                    else {
                        // 没有剩余作品
...
function addDownloadPanel() {
    ...
    centerPanel.outerHTML = `
      ...
      <div class="btns">
      <a class="xztip centerWrap_top_btn update" data-tip="${xzlt('_newver')}" href="https://github.com/xuejianxianzun/PixivBatchDownloader/releases/latest" target="_blank"><img src="${chrome.extension.getURL('images/update.png')}" /></a>
      <a class="xztip centerWrap_top_btn wiki_url" data-tip="${xzlt('_wiki')}" href="https://github.com/xuejianxianzun/PixivBatchDownloader/wiki" target="_blank"><img src="${chrome.extension.getURL('images/wiki.png')}" /></a>
      <a class="xztip centerWrap_top_btn" data-tip="${xzlt('_github')}" href="https://github.com/xuejianxianzun/PixivBatchDownloader" target="_blank"><img src="${chrome.extension.getURL('images/github-logo.png')}" /></a>
      ...
      <button class="startDownload" type="button" style="background:${xzBlue};"> ${xzlt('_下载按钮1')}</button>
      <button class="pauseDownload" type="button" style="background:#e49d00;"> ${xzlt('_下载按钮2')}</button>
      <button class="stopDownload" type="button" style="background:${xzRed};"> ${xzlt('_下载按钮3')}</button>
      <button class="copyUrl" type="button" style="background:${xzGreen};"> ${xzlt('_下载按钮4')}</button>
      </div>
      ...
      <a class="xztip centerWrap_top_btn wiki2" href="https://github.com/xuejianxianzun/PixivBatchDownloader/wiki" target="_blank"><img src="${chrome.extension.getURL('images/wiki.png')}" /> ${xzlt('_wiki')}</a></p>
    ...
    pauseBtn = document.querySelector('.pauseDownload');
    stopBtn = document.querySelector('.stopDownload');
}
// 显示提示
function bindXzTip() {
...
// 向中间面板添加按钮
function addCenterButton(bg = xzBlue, text = '', attr = []) {
...
function manuallyDelete() {
    ...
    addCenterButton(xzRed, xzlt('_手动删除作品'), [
        ...
        delWork = !delWork;
        // 给作品绑定删除属性
        const listElement = document.querySelectorAll(tagSearchListSelector);
...
function showOutputInfoWrap(type) {
    ...
    document.querySelector('.outputInfoWrap').style.display =
        'block';
}
// 生成文件名，传入参数为图片信息
...
chrome.runtime.onMessage.addListener((msg) => {
    if (msg.msg === 'downloaded') {
        ...
        // 下载出错
        addLog(`${imgInfo[msg.data.thisIndex].id}: download error! code: ${msg.err}`, 2);
        reTryDownload();
    }
...
function checkPageType() {
    ...
    else if (!locUrl.includes('mode=manga&illust_id') &&
    ...
    else if (locUrl.includes('ranking_area.php') &&
    ...
    else if (locUrl.includes('https://www.pixivision.net') &&
        locUrl.includes('/a/')) {
        pageType = 8;
    }
    else if (locUrl.includes('bookmark_add.php?id=') ||
        locUrl.includes('bookmark_detail.php?illust_id=')) {
        pageType = 9;
    }
    else if (locUrl.includes('bookmark_new_illust') ||
...
function pageType1() {
    ...
    addCenterButton(xzBlue, xzlt('_从本页开始抓取new')).addEventListener('click', () => {
        downDirection = -1;
        startGet();
    });
    addCenterButton(xzBlue, xzlt('_从本页开始抓取old')).addEventListener('click', () => {
        downDirection = 1;
        startGet();
    });
    ...
    if (pageType === 1 || pageType === 2) {
        ...
        ['pushState', 'popstate'].forEach(item => {
            window.addEventListener(item, () => {
                ...
                addTagBtn = document.getElementById('add_tag_btn');
...
function allPageType() {
    ...
    `;
        ...
        addCenterButton(xzGreen, xzlt('_在结果中筛选'), [
            ...
            const allPicArea = document.querySelectorAll(tagSearchListSelector);
            let wantFavoriteNumber2 = parseInt(window.prompt(xzlt('_在结果中筛选弹窗'), '2000'));
        ...
        if (type === 'illustration' || type === 'manga' || type === 'cosplay') {
            ...
            addCenterButton(xzBlue, xzlt('_抓取该页面的图片')).addEventListener('click', () => {
                resetResult();
                changeTitle('↑');
