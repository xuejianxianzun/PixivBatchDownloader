...
class API {
    ...
    // 获取关注的的新作品的数据
    static getBookmarkNewIllustData(url) {
        return new Promise((resolve, reject) => {
            fetch(url, {
                method: 'get',
                credentials: 'same-origin'
...
class CrawlSearchPage extends CrawlPageBase {
    ...
    initFetchURL() {
        ...
        // 组织必须的信息
        let tempURL = new URL(`https://www.pixiv.net/ajax/search/${pageType}/${encodeURIComponent(word)}?word=${encodeURIComponent(word)}&p=${this.startpageNo}`);
        // 从页面 url 中获取可以使用的参数
        let params = [
            'order',
            'type',
