...
class CrawlBookmarkPage extends _CrawlPageBase__WEBPACK_IMPORTED_MODULE_0__["CrawlPageBase"] {
    ...
    readyGetIdList() {
        ...
        // 判断是公开收藏还是非公开收藏
        const hide = _API__WEBPACK_IMPORTED_MODULE_2__["API"].getURLField(location.href, 'rest') === 'hide';
        // 获取 id 列表
        this.getIdList(hide);
    ...
    // 获取用户的收藏作品列表
    async getIdList(isHide = false) {
        let bmkGetEnd = false; // 书签作品是否获取完毕
        let data;
        try {
            data = await _API__WEBPACK_IMPORTED_MODULE_2__["API"].getBookmarkData(_DOM__WEBPACK_IMPORTED_MODULE_5__["DOM"].getUserId(), this.tag, this.offset, isHide);
        }
        catch (error) {
...
class CrawlPageBase {
    ...
    checkWantPageInputGreater0() {
        const result = _API__WEBPACK_IMPORTED_MODULE_2__["API"].checkNumberGreater0(_Settings__WEBPACK_IMPORTED_MODULE_7__["form"].setWantPage.value);
        if (result.result) {
            return result.value;
        }
        else {
    ...
    // 获取作品张数设置
    getFirstFewImages() {
        const check = _API__WEBPACK_IMPORTED_MODULE_2__["API"].checkNumberGreater0(_Settings__WEBPACK_IMPORTED_MODULE_7__["form"].firstFewImages.value);
        if (check.result) {
            return check.value;
        }
        else {
    ...
    getMultipleSetting() {
        this.multipleImageWorks = parseInt(_Settings__WEBPACK_IMPORTED_MODULE_7__["form"].multipleImageWorks.value);
    ...
    async readyCrawl() {
        ...
        _TitleBar__WEBPACK_IMPORTED_MODULE_8__["titleBar"].changeTitle('↑');
        _CenterPanel__WEBPACK_IMPORTED_MODULE_6__["centerPanel"].close();
        this.getWantPage();
        _Filter__WEBPACK_IMPORTED_MODULE_0__["filter"].init();
        this.getMultipleSetting();
        await _PageInfo__WEBPACK_IMPORTED_MODULE_9__["pageInfo"].store();
        // 进入第一个抓取方法
        this.nextStep();
    }
    ...
    async getWorksData(id) {
        ...
        if (_Filter__WEBPACK_IMPORTED_MODULE_0__["filter"].check(filterOpt)) {
            ...
            if (body.illustType !== 2) {
                ...
                const ext = _Settings__WEBPACK_IMPORTED_MODULE_7__["form"].ugoiraSaveAs.value; // 扩展名可能是 webm、gif、zip
                _Store__WEBPACK_IMPORTED_MODULE_3__["store"].addResult({
                    id: illustId,
                    idNum: idNum,
                    thumb: thumb,
    ...
    crawlFinished() {
        ...
        if (!_Store__WEBPACK_IMPORTED_MODULE_3__["store"].states.quickDownload) {
            _CenterPanel__WEBPACK_IMPORTED_MODULE_6__["centerPanel"].show();
        }
    }
    // 网络请求状态异常时输出提示
...
class InitBookmarkPage extends _InitPageBase__WEBPACK_IMPORTED_MODULE_0__["InitPageBase"] {
    ...
    appendCenterBtns() {
        ...
        if (_API__WEBPACK_IMPORTED_MODULE_1__["API"].getToken()) {
            let btn = _CenterButtons__WEBPACK_IMPORTED_MODULE_4__["centerButtons"].add(_Colors__WEBPACK_IMPORTED_MODULE_2__["Colors"].green, _Lang__WEBPACK_IMPORTED_MODULE_3__["lang"].transl('_添加tag'), [
                ['title', _Lang__WEBPACK_IMPORTED_MODULE_3__["lang"].transl('_添加tag')]
            ]);
            btn.id = 'add_tag_btn';
            _BookmarksAddTag__WEBPACK_IMPORTED_MODULE_6__["bookmarksAddTag"].init(btn);
        }
    }
    setFormOption() {
        // 设置“个数/页数”选项
