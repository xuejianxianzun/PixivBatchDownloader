...
class InitSetting {
    ...
    restoreOption() {
        ...
        // 设置作品张数
        ui.form.imgNumberPerWork.value = (this.needSaveOpts.imgNumberPerWork | 0).toString();
        // 设置排除类型
...
class Store {
     ...
     */
    addResult(id, url, title, tags, tagsTranslated, user, userid, fullWidth, fullHeight, ext, bmk, date, type, rank, ugoiraInfo) {
        this.result.push({
            id,
            url,
            title,
        ...
        if (totalCheck) {
            ...
            if (jsInfo.illustType !== 2) {
                ...
                for (let i = 0; i < pNo; i++) {
                    ...
                    store.addResult(illustId + '_p' + i, nowUrl, title, nowAllTag, tagWithTranslation, user, userid, fullWidth, fullHeight, ext, bmk, jsInfo.createDate.split('T')[0], jsInfo.illustType, rank, {});
                }
                this.outputImgNum();
            }
                ...
                store.addResult(illustId, info.body.originalSrc, title, nowAllTag, tagWithTranslation, user, userid, fullWidth, fullHeight, ext, bmk, jsInfo.createDate.split('T')[0], jsInfo.illustType, rank, ugoiraInfo);
                this.outputImgNum();
            }
        }
        // 在作品页内下载时，每次获取完一个作品的信息后，还要从这个信息里获取下一个要抓取的 id
        if (this.type === 1 && !this.downRelated) {
            if (this.fetchNumber > 0) {
                this.fetchNumber--;
            }
...
class CrawlIllustPage extends CrawlPageBase {
    getWantPage() {
        if (dlCtrl.quickDownload) {
            ...
            // 检查下载页数的设置
            if (!this.downRelated) {
                this.fetchNumber = setting.checkWantPageInput(lang.transl('_checkWantPageRule1Arg3'), lang.transl('_checkWantPageRule1Arg4'));
            }
            else {
                // 相关作品的提示
                this.fetchNumber = setting.checkWantPageInput(lang.transl('_checkWantPageRule1Arg9'), lang.transl('_checkWantPageRule1Arg10'));
            }
        }
    }
    getListPage() {
...
class CrawlUserPage extends CrawlPageBase {
    ...
    getWantPage() {
        ...
        this.fetchNumber = setting.checkWantPageInput(lang.transl('_checkWantPageRule1Arg6'), pageTip);
    }
    nextStep() {
        // 下载推荐图片
        if (this.downRecommended) {
    ...
    afterGetListPage() {
        ...
        log.log(lang.transl('_列表抓取完成开始获取作品页', store.idList.length.toString()));
        this.getListUrlFinished();
    }
    // 获取用户的全部作品列表
    ...
    async getBookmarkList(restMode) {
        ...
        let data;
        try {
            data = await API.getBookmarkData(DOM.getUserId(), this.tag, this.offset, restMode);
        }
        catch (error) {
...
class CrawlSearchPage extends CrawlPageBase {
    getWantPage() {
        this.fetchNumber = setting.checkWantPageInput(lang.transl('_checkWantPageRule1Arg6'), lang.transl('_checkWantPageRule1Arg7'));
    ...
    initFetchURL() {
        ...
        // 组织必须的信息
        let tempURL = new URL(`https://www.pixiv.net/ajax/search/${pageType}/${word}?word=${word}&p=${this.startpageNo}`);
        // 从页面 url 中获取可以使用的参数
        let params = [
            'order',
            'type',
    ...
    async getListPage() {
        ...
        if (this.listPageFinished === this.fetchNumber ||
...
class CrawlRankingPage extends CrawlPageBase {
    ...
    getWantPage() {
        this.listPageFinished = 0;
        // 检查下载页数的设置
        this.fetchNumber = setting.checkWantPageInput(lang.transl('_checkWantPageRule1Arg12'), lang.transl('_checkWantPageRule1Arg4'));
...
class CrawlPixivisionPage extends CrawlPageBase {
        ...
        if (type === 'illustration') {
            ...
            Array.from(imageList).forEach(el => {
                ...
                if (imgUrl !== 'https://i.pximg.net/imgaz/upload/20170407/256097898.jpg') {
                    ...
                    store.addResult(id, imgUrl, '', [], [], '', '', 0, 0, ext, 0, '', 0, '', {});
                }
            });
            this.crawFinished();
        }
    ...
    testExtName(url, imgNumber, imgInfoData) {
        ...
        let next = (bool) => {
            ...
            store.addResult(imgInfoData.id, url, imgInfoData.title, imgInfoData.tags, [], imgInfoData.user, imgInfoData.userid, imgInfoData.fullWidth, imgInfoData.fullHeight, ext, 0, '', 0, '', {});
            this.outputImgNum();
...
class CrawlBookmarkNewIllustPage extends CrawlPageBase {
    ...
    getListPage() {
        ...
        fetch(url)
            ...
            // 保存本页的作品信息
            let worksData = JSON.parse(worksInfoText);
...
class CrawlNewIllustPage extends CrawlPageBase {
    constructor() {
        ...
        this.limit = 20; // 每次请求的数量，可以比 20 小
    ...
    async getListPage() {
        ...
        let data = await API.getNewIllustData(tempURL.toString());
        ...
        // 抓取完毕
        if (this.fetchCount >= this.fetchNumber ||
...
class InitSearchPage extends InitPageBase {
    ...
    fastScreen() {
        ...
        const tag = API.getURLField(location.href, 'word');
        const firstTag = encodeURIComponent(decodeURIComponent(tag).split(' ')[0]);
        const favNums = [
            '100users入り',
            '500users入り',
            '1000users入り',
        ...
        fastScreenArea.innerHTML = favNums.reduce((result, cur) => {
            return (result += `<a href="https://www.pixiv.net/search.php?word=${firstTag}%20${cur}">${cur}</a>`);
        }, '');
    ...
    GetCrawler(type) {
        ...
        switch (type) {
            ...
            case 9:
                ...
                break;
            case 10:
                result = new CrawlBookmarkNewIllustPage();
                break;
            case 11:
    ...
    initPage() {
        ...
        switch (this.type) {
            ...
            case 9:
                ...
                break;
            case 10:
                result = new InitBookmarkNewIllustPage(crawler);
                break;
            case 11:
    ...
    listenDownloaded() {
        chrome.runtime.onMessage.addListener((msg) => {
            ...
            if (msg.msg === 'downloaded') {
                ...
                // 下载出错
                log.error(`${store.result[msg.data.thisIndex].id}: download error! code: ${msg.err}`);
