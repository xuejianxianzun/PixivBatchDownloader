...
class Bookmark {
    ...
    static async add(id, type, tags) {
        if (!tags) {
            ...
            if (_setting_Settings__WEBPACK_IMPORTED_MODULE_1__["settings"].widthTagBoolean) {
                const data = await this.getWorkData(type, id);
                tags = _Tools__WEBPACK_IMPORTED_MODULE_3__["Tools"].extractTags(data);
            }
        }
        return _API__WEBPACK_IMPORTED_MODULE_0__["API"].addBookmark(type, id, tags, _setting_Settings__WEBPACK_IMPORTED_MODULE_1__["settings"].restrictBoolean, _Token__WEBPACK_IMPORTED_MODULE_2__["token"].token);
    }
}
...
class FileName {
    ...
    getFileName(data) {
        ...
        // 判断这个作品是否要去掉序号
        const noSerialNo = _setting_Settings__WEBPACK_IMPORTED_MODULE_0__["settings"].noSerialNo && cfg['{p_num}'].value === 0;
        // 2 替换命名规则里的特殊字符
        ...
        for (let index = 0; index < pathArray.length; index++) {
            ...
            str = _utils_Utils__WEBPACK_IMPORTED_MODULE_4__["Utils"].handleWindowsReservedName(str, this.addStr);
            pathArray[index] = str;
        }
...
class ImageViewer {
    constructor(cfg) {
        ...
        this.bookmarkBtnClass = 'viewer-bookmark-btn';
    ...
    addBookmarkBtn() {
        ...
        if (one2one) {
            const li = document.createElement('li');
            li.setAttribute('role', 'button');
            li.setAttribute('title', _Lang__WEBPACK_IMPORTED_MODULE_2__["lang"].transl('_收藏') + ' (B)');
            ...
            btn.addEventListener('click', () => {
                // this.download()
                window.setTimeout(() => {
                    _Toast__WEBPACK_IMPORTED_MODULE_6__["toast"].success(_Lang__WEBPACK_IMPORTED_MODULE_2__["lang"].transl('_已加入收藏'));
                }, 500);
            });
        }
    }
...
class InitSearchArtworkPage extends _crawl_InitPageBase__WEBPACK_IMPORTED_MODULE_0__["InitPageBase"] {
    ...
    addCrawlBtns() {
        _Tools__WEBPACK_IMPORTED_MODULE_12__["Tools"].addBtn('crawlBtns', _config_Colors__WEBPACK_IMPORTED_MODULE_1__["Colors"].bgGreen, _Lang__WEBPACK_IMPORTED_MODULE_2__["lang"].transl('_开始筛选'), [
    ...
    destroy() {
        ...
        // 离开下载页面时，取消设置“不自动下载”
        _store_States__WEBPACK_IMPORTED_MODULE_14__["states"].notAutoDownload = false;
    }
    getWantPage() {
    ...
    async nextStep() {
        ...
        if (this.startpageNo > 1000) {
            _MsgBox__WEBPACK_IMPORTED_MODULE_18__["msgBox"].error(`${_Lang__WEBPACK_IMPORTED_MODULE_2__["lang"].transl('_超出最大页码')} ${this.maxCount}`);
            return this.noResult();
        }
    ...
    setNotAutoDownload() {
        ...
        _store_States__WEBPACK_IMPORTED_MODULE_14__["states"].notAutoDownload = _setting_Settings__WEBPACK_IMPORTED_MODULE_10__["settings"].previewResult ? true : false;
    }
    // 返回包含作品列表的 ul 元素
    getWorksWrap() {
...
class BookmarkAfterDL {
    ...
    async addBookmark(id) {
        return new Promise(async (resolve, reject) => {
            ...
            // 从 store 里查找这个作品的数据
            let data = undefined;
            let dataSource = _store_Store__WEBPACK_IMPORTED_MODULE_0__["store"].resultMeta.length > 0 ? _store_Store__WEBPACK_IMPORTED_MODULE_0__["store"].resultMeta : _store_Store__WEBPACK_IMPORTED_MODULE_0__["store"].result;
...
class BookmarkAllWorks {
    ...
    complete() {
        ...
        _EVT__WEBPACK_IMPORTED_MODULE_2__["EVT"].fire(_EVT__WEBPACK_IMPORTED_MODULE_2__["EVT"].list.bookmarkModeEnd);
...
class BookmarksAddTag {
    ...
    async addTag() {
        ...
        await _API__WEBPACK_IMPORTED_MODULE_0__["API"].addBookmark(this.type, item.id, item.tags, item.restrict, _Token__WEBPACK_IMPORTED_MODULE_2__["token"].token);
