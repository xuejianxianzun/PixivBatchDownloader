    ...
    async download(arg) {
        ...
        xhr.addEventListener('loadend', async () => {
            ...
            if (xhr.status !== 200) {
                ...
                if ((convertExt.includes(arg.data.ext)) &&
                    arg.data.ugoiraInfo) {
                    try {
    ...
    async createPngBuffer(img, size = [16, 48, 96, 256], shape = 'square', bleed = true) {
        return new Promise(async (resolve, reject) => {
            ...
            while (length > 0) {
                ...
                const buf2 = (_a = canvas.getContext('2d')) === null || _a === void 0 ? void 0 : _a.getImageData(0, 0, sizeNumber, sizeNumber).data.buffer;
...
class SaveNamingRule {
  ...
  <ul class="namingRuleList"></ul>
        ...
        _DOM__WEBPACK_IMPORTED_MODULE_0__["DOM"].useSlot('saveNamingRule', this.html);
...
class ToAPNG {
    async convert(file, info) {
        return new Promise(async (resolve, reject) => {
            // 获取解压后的图片数据
            let base64Arr = await _ExtractImage__WEBPACK_IMPORTED_MODULE_0__["extractImage"].extractImageAsDataURL(file, info).catch(() => {
