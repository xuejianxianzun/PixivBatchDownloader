...
class UI {
    ...
    addCenterPanel() {
      ...
      <p class="formOption3">
      <span class="has_tip settingNameStyle1" data-tip="${lang.transl('_多p下载前几张提示')}">${lang.transl('_多p下载前几张')}<span class="gray1"> ? </span></span>
      ...
      <input type="radio" name="ratio" id="ratio3" value="3"> <label for="ratio3"> ${lang.transl('_输入宽高比')}<input type="text" name="userRatio" class="setinput_style1 blue" value="1.4"></label>
      </p>
      <p class="formOption6">
      ...
      `;
        ...
        this.downloadArea = document.querySelector('.download_panel');
    ...
    // 获取排除类型
    getNotDownType() {
        let result = Array.from(ui.form.querySelectorAll('.formOption5 input')).reduce((result, el, index) => {
...
class DownloadFile {
    ...
    download(progressBarNo) {
        ...
        xhr.addEventListener('loadend', async () => {
            ...
            else if ((data.ext === 'webm' || data.ext === 'gif') &&
                data.ugoiraInfo) {
                file = xhr.response;
                // 如果需要转换成视频
                if (data.ext === 'webm') {
