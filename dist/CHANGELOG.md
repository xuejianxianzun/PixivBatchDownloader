...
fmt 命令使用 prettier 命令行进行格式化。之前没有指定要格式的文件的后缀名，导致会格式化到 png 文件，产生警告。这次由于增加了 GitHub 的赞助文件，prettier 格式化出错了，所以现在指定了要格式的文件的后缀名。
...
1. 可能会误伤（但关系不大）。灰色图片的 R G B 三色应该是相同的颜色，例如 `200 200 200`。但是我加了一个数值为 1 的宽容度，这样类似于 `200 200 201` 的图片也会被认为是黑白图片。所以偶尔会误伤，但情况很少；而且即使误伤了，这样的图片也很接近黑白图片，关系不大。
2. 对于整体是黑白线稿，只在部分涂抹了一点颜色的图片来说，大部分会被判断为彩色图片。如果用户排除黑白图片，排除不了这种。这很遗憾。
3. 如果一张图是黑白线稿，但背景色是彩色的图，大部分会被判断为彩色图片。如果用户排除黑白图片，排除不了这种。例如 `https://www.pixiv.net/artworks/80469066`，这图**看上去**第一感觉像是黑白的，但是这图的背景色有点偏紫色，所以**事实上**这不是黑白图，程序会把它判断为彩色图片。虽然和人的期望不一样，但是也没什么好的办法。
4. 如果一个多图作品，第一张图的颜色和后面图片不同，可能会有误判。假如一个作品第一张是黑白图，后面是彩图，用户设置不下载黑白图，那么由于第一张图不符合要求，所以整个作品就全被排除掉了。这会造成失误，但是这样的作品很少。第一张是彩图后面是黑白图的比较多，而大部分人是喜欢保留彩图的，所以此时不会造成误判。

第一张是彩图，后面是黑白图的示例：

`https://www.pixiv.net/artworks/80417958`
